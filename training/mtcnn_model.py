...
def P_Net(inputs, label=None, bbox_target=None, landmark_target=None, training=True):
    ...
    with slim.arg_scope([slim.conv2d],
                        ...
                        padding='valid'):
        net = slim.conv2d(inputs, landmark_number, 3, stride=1,scope='conv1')
        ...
        #batch*H*W*10
        landmark_pred = slim.conv2d(net, num_outputs=landmark_number, kernel_size=[1,1], stride=1, scope='conv4_3', activation_fn=None)
        if training:
            #batch*2
...
def R_Net(inputs,label=None, bbox_target=None, landmark_target=None, training=True):
    with slim.arg_scope([slim.conv2d],
        ...
        #batch*10
        landmark_pred = slim.fully_connected(fc1,num_outputs=landmark_number,scope="landmark_fc",activation_fn=None)
        #train
        if training:
            cls_loss = cls_ohem(cls_prob,label)
            bbox_loss = bbox_ohem(bbox_pred,bbox_target,label)
