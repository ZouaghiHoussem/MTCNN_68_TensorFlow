...
def save_bboxes(save_path,face_idnex,image, bboxes):
    for index,bbox in enumerate(bboxes):
        print("bbox {}".format(bboxes))
        image_rect = image[int(bbox[1]):int(bbox[3]),int(bbox[0]):int(bbox[2])]
        cv2.imwrite(os.path.join(save_path, "face_{}_bbox_{}.jpg".format(face_idnex,index)), image_rect)
